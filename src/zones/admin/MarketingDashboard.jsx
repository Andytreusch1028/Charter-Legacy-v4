...
import { X, Send, Megaphone, Activity, CheckCircle2, AlertTriangle, Key, Edit3 } from 'lucide-react';
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-gradient-to-br from-[#1D1D1F] to-[#121213]">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">Broadcasting Queue</h3>
            <span className="bg-white/10 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase">{queue.length} Pending</span>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 border-2 border-dashed border-white/5 rounded-[32px]">
               <CheckCircle2 className="text-white/20 mb-4" size={48} />
               <p className="text-white/40 text-sm font-medium">All narratives deployed.</p>
            </div>
          ) : (
                  ...
                  <div className="flex flex-col lg:flex-row gap-8 relative z-10">
                    <div className="flex-1 space-y-4">
                      ...
                      {editingId === item.id ? (
                        ...
                        <div className="group/text cursor-pointer" onClick={() => handleStartEdit(item)}>
                          <div className="absolute top-8 right-8 text-white/0 group-hover/text:text-white/40 transition-colors">
                             <Edit3 size={16} />
                          </div>
                          <p className="text-white/80 whitespace-pre-wrap text-sm leading-relaxed p-4 border border-transparent group-hover/text:bg-white/5 group-hover/text:border-white/10 rounded-xl transition-all">
