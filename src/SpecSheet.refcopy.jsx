...
const DetailsStep = ({ item, boiAdded, setBoiAdded, breachAdded, setBreachAdded, loading, onNext }) => {
    
    const totalPrice = useMemo(() => {
        if (!item) return '$0';
        if (item.id === 'will') return item.price;
        
        const basePrice = parseInt(item.price.replace(/[^0-9]/g, '')) || 0;
        const boiCost = (item.id !== 'sovereign_upgrade' && boiAdded) ? 149 : 0;
        const breachCost = (item.id !== 'sovereign_upgrade' && breachAdded) ? 49 : 0;
        
        return `$${basePrice + boiCost + breachCost}`;
    }, [item, boiAdded, breachAdded]);
    ...
    return (
        <div className="space-y-8 animate-in slide-in-from-right duration-500">
             ...
             <button onClick={onNext} disabled={loading} className="w-full bg-[#007AFF] text-white py-5 rounded-2xl font-black text-sm uppercase tracking-[0.2em] shadow-xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3">
...
const SetupStep = ({ email, setEmail, password, setPassword, error, loading, onSetup, onMagicLink }) => {
    return (
        <div className="space-y-8 animate-in slide-in-from-right duration-500">
             <div>
                ...
                <div className="p-4 bg-red-50 text-red-600 text-xs font-bold rounded-xl animate-in slide-in-from-top-2">
                    ...
                    {error.includes("Account exists") && !error.includes("Magic Link") && (
                         <div className="mt-3 pt-3 border-t border-red-100">
                             <button onClick={onMagicLink} className="text-[#007AFF] underline hover:text-black transition-colors block w-full text-left">
             ...
             <button onClick={onSetup} disabled={loading} className="w-full bg-black text-white py-5 rounded-2xl font-black text-sm uppercase tracking-[0.2em] shadow-xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3">
...
const PaymentStep = ({ item, error, setError, clientSecret, loading, onMockPayment, onPaymentSuccess }) => {
    return (
        <div className="space-y-8 animate-in slide-in-from-right duration-500">
             <div>
             ...
             <div className="p-6 bg-[#F5F5F7] rounded-3xl space-y-4 border border-gray-100">
               ...
               {clientSecret ? (
                  <Elements stripe={stripePromise} options={{ clientSecret, appearance: { theme: 'stripe', labels: 'floating' } }}>
                     <CheckoutForm onSuccess={onPaymentSuccess} onError={setError} />
                  </Elements>
               ) : (
                  <>
                    ...
                    <button onClick={onMockPayment} disabled={loading} className="w-full bg-[#007AFF] text-white py-5 rounded-2xl font-black text-sm uppercase tracking-[0.2em] shadow-xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3">
...
const SuccessStep = ({ user, onSuccess }) => {
    return (
      ...
      <div className="bg-white w-full max-w-xl rounded-[48px] p-12 shadow-2xl relative overflow-hidden animate-in zoom-in-95 text-left border border-gray-100 transition-all duration-500">
        ...
        {step === 'details' && (
            <DetailsStep 
                item={item} 
                boiAdded={boiAdded} 
                setBoiAdded={setBoiAdded} 
                breachAdded={breachAdded} 
                setBreachAdded={setBreachAdded} 
                loading={loading} 
                onNext={handleNext} 
            />
        )}
