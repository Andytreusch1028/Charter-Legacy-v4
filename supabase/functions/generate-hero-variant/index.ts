...
serve(async (req) => {
  ...
  try {
    ...
    if (!openAiKey) {
      // Fallback to mock data if no OpenAI key is configured
      variants = [
        { headline: "Your Legacy, Secured Today.", subheading: "Setup your LLC and protect your assets in under 10 minutes. No jargon, just pure execution.", target_sentiment: "Ease of Use" },
        { headline: "Total Privacy. Zero Compromises.", subheading: "Anonymous by default. Keep your personal details off public records while we handle the heavy lifting.", target_sentiment: "Privacy" }
      ]
    } else {
        const systemPrompt = `
