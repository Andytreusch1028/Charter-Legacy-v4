...
// ΓöÇΓöÇΓöÇ Document Classification Categories (research-backed) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
// Source: Florida Registered Agent document types per FL Stat. ┬º48, ┬º605, ┬º607
...
// ΓöÇΓöÇΓöÇ Module Config ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const MODULES = [
...
// Current operator ΓÇö will be fetched from Supabase Auth in production
...
const FulfillmentPortal = () => {
    ...
    // ΓöÇΓöÇ REAL OCR TRIGGER ΓÇö Runs when a local document is selected ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    useEffect(() => {
    ...
    // Pre-classifier effect ΓÇö runs once clients are loaded
    useEffect(() => {
    ...
    // ΓöÇΓöÇ Track when staff starts reviewing an item (for review_time_ms) ΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Calculate review time for an item ΓöÇΓöÇ
    const getReviewTimeMs = useCallback((itemId) => {
        const start = reviewStartTimes[itemId];
        return start ? Date.now() - start : 0;
    }, [reviewStartTimes]);

    // ΓöÇΓöÇ ACCEPT MATCH ΓÇö AI was correct, staff confirms ΓöÇΓöÇ
    const handleAcceptMatch = useCallback((itemId) => {
        ...
        if (classification) {
            ...
            }).catch(() => {}); // Silently fail ΓÇö feedback is best-effort
        }
        ...
        setToast({ type: 'success', message: `Γ£ô AI match confirmed ΓÇö model weights reinforced` });
    }, [queue, aiClassifications, linkedEntities, getReviewTimeMs]);

    // ΓöÇΓöÇ OVERRIDE ΓÇö AI was wrong, staff rejects and will manually link ΓöÇΓöÇ
    const handleOverride = useCallback((itemId) => {
        const item = queue.find(q => q.id === itemId);
        const classification = aiClassifications[itemId];
        if (!item) return;

        // Update queue state ΓÇö clear entity data
        ...
        setToast({ type: 'info', message: `Override recorded ΓÇö AI weights adjusted to reduce future mismatches` });
    }, [queue, aiClassifications, getReviewTimeMs]);

    // ΓöÇΓöÇ MANUAL LINK ΓÇö Staff manually selects an entity (for needs_review items) ΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ AUTHENTIC SCAN FOLDER ΓÇö Pulls from real local file buffer ΓöÇΓöÇ
                        ...
                        <div className="bg-gradient-to-br from-violet-50 to-purple-50 rounded-xl p-3">
                            ...
                            <div className="flex items-baseline gap-2">
                                <span className="text-2xl font-black text-violet-700">
                                    {aiStats.total > 0 ? `${Math.round((aiStats.accepted / aiStats.total) * 100)}%` : 'ΓÇö'}
                                </span>
                        ...
                        <div className="border-t border-violet-100 pt-3">
                            ...
                            <p className="text-[9px] text-gray-500 leading-relaxed">
                                {feedbackBuffer.length === 0 
                                    ...
                                    : `${feedbackBuffer.length} feedback events buffered. Each Accept reinforces matching rules (+╬┤). Each Override penalizes incorrect rules (-╬┤). Weights converge toward higher accuracy over time.`
                                }
                            </p>
                        </div>
                        ...
                        <main className="flex-1 bg-white border border-gray-100 rounded-[32px] flex flex-col overflow-hidden shadow-sm">
                            ...
                            <div className="flex-1 overflow-y-auto p-8 space-y-6 flex flex-col-reverse">
                                ...
                                <div className="max-w-[80%] self-start space-y-2">
                                    ...
                                    <p className="text-[8px] font-bold text-gray-400 ml-1 uppercase">A. Lozano (Operator) ΓÇó 2h ago</p>
                                </div>
                                ...
                                <div className="max-w-[80%] self-end space-y-2">
                                    ...
                                    <p className="text-[8px] font-bold text-gray-400 mr-1 text-right uppercase">System ΓÇó 45m ago</p>
                                </div>
                            </div>
                        ...
                        {queue.length === 0 ? (
                            ...
                            >
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className="text-[8px] font-mono text-gray-300">{item.hubId || 'ΓÇö'}</span>
                                </div>
                                ...
                                <p className="text-[9px] text-gray-400 font-medium italic mb-3 truncate">{item.entity ? item.docTitle : 'Unclassified ΓÇö manual review required'}</p>
                                <div className="flex items-center justify-between">
                                ...
                                <div className="p-5 border-b border-gray-100 flex items-center justify-between shrink-0">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{doc.entity ? `${doc.entity} (${doc.sunbizId})` : 'Unlinked ΓÇö entity match required'}</p>
                                        </div>
                                    </div>
                                            ...
                                            <div className={`p-3 rounded-2xl border ${
                                                ...
                                                <div className="flex items-center justify-between mb-2">
                                                    <p className={`text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 ${
                                                        ...
                                                        {doc.aiConfidence >= 80 ? 'Γ£ô AI Matched' : doc.aiConfidence >= 50 ? 'Γùï AI Partial' : 'ΓÜá Needs Review'}
                                                    </p>
                                                ...
                                                {doc.aiConfidence >= 50 ? (
                                                    ...
                                                    <p className="text-[8px] font-black text-amber-600 uppercase tracking-widest">Link entity manually below Γåô</p>
                                                )}
                                            </div>
                                        )}
                                        {doc.aiStatus === 'confirmed' && (
                                         ...
                                         <div className="space-y-4">
                                            ...
                                            <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100 space-y-3">
                                                ...
                                                <div className="space-y-2">
                                                    <div className="flex justify-between items-center">
                                                        ...
                                                        <span className="text-[9px] font-mono text-gray-500">{(doc.meta?.size) || 'ΓÇö'}</span>
                                                    </div>
                                                    <div className="flex justify-between items-center">
                                            ...
                                            <button 
                                                onClick={(e) => {
                                                    ...
                                                    e.currentTarget.querySelector('span').textContent = el.style.display === 'none' ? 'Γû╕' : 'Γû╛';
                                                }}
                                            ...
                                            <div style={{ display: 'none' }} className="space-y-1 pl-2 border-l-2 border-gray-100">
                                                <div className="space-y-0.5">
                                                    ...
                                                    <p className="text-[9px] font-mono font-medium text-gray-400">{doc.hubId || 'ΓÇö unassigned'}</p>
                                                </div>
                                                <div className="space-y-0.5">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="grid grid-cols-2 gap-1.5">
                                                {[...DOCUMENT_CATEGORIES, ...customCategories.map(c => ({ id: `custom_${c}`, label: c, custom: true }))].map(cat => (
                                                    ...
                                                    >
                                                        {cat.urgent && <span className="mr-0.5">ΓÜá</span>}{cat.label}
                                                        {cat.custom && (
                                                            <span 
                                            ...
                                            {/* OTHER ΓÇö custom category input */}
                                            {!showOtherInput[doc.id] ? (
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                    <input 
                                                        ...
                                                        placeholder="Type classificationΓÇª" 
